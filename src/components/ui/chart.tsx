...
type ChartTooltipProps = React.ComponentProps<"div"> & {
  ...
  payload?: RechartsPrimitive.DefaultTooltipContentProps<
    ...
    ChartNameType
  >["payload"];
  label?: RechartsPrimitive.TooltipContentProps<
    ChartValueType,
    ChartNameType
  >["label"];
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "bg-secondary-bg/55 text-primary-text border-border-card grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl backdrop-blur-3xl",
          className,
        )}
      >
  ...
  (
    ...
    return (
      ...
      >
        {payload
          .filter(
            (item: RechartsPrimitive.LegendPayload) => item.type !== "none",
          )
