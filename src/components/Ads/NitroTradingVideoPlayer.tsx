...
const SLOT_ID = "np-trading-video";
...
export default function NitroTradingVideoPlayer({ className }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      Promise.resolve(nitroAds.createAd(SLOT_ID, CONFIG)).catch((error) => {
        log.warn("[Nitro Ad] Failed to create trading video player ad:", error);
        createdRef.current = false;
      });
    } catch (error) {
      log.warn("[Nitro Ad] Error initializing trading video player ad:", error);
      createdRef.current = false;
    }
