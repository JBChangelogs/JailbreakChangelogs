...
const SLOT_ID = "np-values-suggestions-video";
...
export default function NitroValuesSuggestionsVideoPlayer({
  className,
}: Props) {
  ...
  useEffect(() => {
    ...
    try {
      Promise.resolve(nitroAds.createAd(SLOT_ID, CONFIG)).catch((error) => {
        log.warn(
          "[Nitro Ad] Failed to create suggestions video player ad:",
          error,
        );
        createdRef.current = false;
      });
      ...
      log.warn(
        "[Nitro Ad] Error initializing suggestions video player ad:",
        error,
      );
      createdRef.current = false;
    }
