...
export default function DupeFinderResults({ 
  ...
  const [sortOrder, setSortOrder] = useState<'alpha-asc' | 'alpha-desc' | 'traded-desc' | 'unique-desc' | 'created-asc' | 'created-desc'>('created-desc');
  ...
  }, [initialData, fetchMissingUserData, fetchOriginalOwnerAvatarsData, localRobloxUsers, localRobloxAvatars]);
  ...
  if (!initialData || initialData.length === 0) {
    return (
      <div className="bg-[#212A31] rounded-lg p-6 shadow-sm border border-[#2E3944]">
        <div className="text-center">
          <div className="text-gray-400 text-lg font-medium mb-2">No Dupe Items Found</div>
          <div className="text-gray-500">This user doesn&apos;t have any duped items in our database.</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#212A31] rounded-lg p-6 shadow-sm border border-[#2E3944]">
        ...
        {/* Stats */}
        <div className="text-center">
          <div className="text-sm text-muted">Dupe Items Found</div>
          <div className="text-2xl font-bold text-[#ef4444]">{initialData.length?.toLocaleString()}</div>
        </div>
      </div>

      {/* Results */}
      <div className="bg-[#212A31] rounded-lg p-6 shadow-sm border border-[#2E3944]">
        <h2 className="text-xl font-semibold text-muted mb-4">Dupe Items</h2>
        ...
        {filteredData.length === 0 && (searchTerm || selectedCategories.length > 0) && (
        ...
        {(searchTerm || selectedCategories.length > 0) && filteredData.length > 0 && (
