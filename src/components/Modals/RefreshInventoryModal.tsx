...
interface RefreshInventoryModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function RefreshInventoryModal({
  isOpen,
  onClose,
  onSuccess,
  isRefreshing,
}: RefreshInventoryModalProps) {
  const handleTurnstileSuccess = (token: string) => {
    // Automatically proceed with refresh when token is obtained
    onSuccess(token);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-secondary-bg border-border-primary shadow-card-shadow w-full max-w-md rounded-lg border p-6">
        ...
        <p className="text-secondary-text mb-6 text-sm">
          Complete the security check below to refresh your inventory data.
        </p>
