...
export async function checkNetworthLeaderboardMaintenanceMode(): Promise<{
  isNetworthLeaderboardMaintenanceMode: boolean;
}> {
  const isNetworthLeaderboardMaintenanceMode =
    process.env.NEXT_PUBLIC_NETWORTH_LEADERBOARD_MAINTENANCE_MODE === "true";

  return {
    isNetworthLeaderboardMaintenanceMode,
  };
}

export async function getNetworthLeaderboardMaintenanceMetadata(): Promise<Metadata | null> {
  const { isNetworthLeaderboardMaintenanceMode } =
  ...
  if (isNetworthLeaderboardMaintenanceMode) {
    return {
      metadataBase: new URL("https://jailbreakchangelogs.xyz"),
      title: "Networth Leaderboard - Under Maintenance",
      description:
        "The Networth Leaderboard is currently under maintenance. We'll be back soon!",
      robots: {
        index: false,
        follow: false,
      },
        ...
        images: [
          {
            ...
            width: 1200,
            height: 630,
            alt: "Jailbreak Changelogs Networth Leaderboard Maintenance Banner",
          },
        ],
      },
      twitter: {
